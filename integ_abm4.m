...
for t = upper+t_step : t_step : t_stop-t_step,
    ...
    % 4-step Adams - Bashforth method is used as a predictor:
    sd = feval(model, s, t, param);
    p = s + t_step * (55*sd - 59*sd1 + 37*sd2 - 9*sd3) / 24;
    
    % and corrected by the 4-step Adams - Moulton method:
    pd = feval(model, p, t+t_step, param);
    s = s + t_step * (251*pd + 646*sd - 264*sd1 + 106*sd2 - 19*sd3) / 720;
