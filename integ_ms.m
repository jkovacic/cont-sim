...
function output = integ_ms(model, initial_condition, t_start, t_stop, t_step, outputf, param)
...
[outrk4, S, H] = aux_rk4(model, initial_condition, t_start, upper_limit, t_step, outputf, param, 4);
s = S(:, 1:STATE_COLS);
...
for t = upper_limit+t_step : t_step : t_stop-t_step
    ...
    s = S(:, (STATE_COLS+1) : (2*STATE_COLS) ) + t_step * ( H(:, 1:STATE_COLS ) + 4*sd + pd ) / 3;
