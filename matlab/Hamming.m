classdef Hamming < IntegMultiStepAb
    % A differential equation solver, based on the Hamming's 
    % predictor - corrector method. The method is described at:
    % http://www-solar.mcs.st-and.ac.uk/~steveb/course/notes/set6_2.pdf
    
    
    methods
        function obj = Hamming(model, input, t_start, t_stop, t_step, init_condition)
            % Constructor.
            %
            % Input:
        ...
        function output = run(self)
            ...
            for t = upper_limit+self.t_step : self.t_step : self.t_stop-self.t_step
                ...
                s = ( 9 * s - S(:, (2*STATE_COLS+1) : (3*STATE_COLS) ) ) / 8 + ...
                    3 * self.t_step * ( pd + 2*sd - H(:, 1:STATE_COLS) ) / 8;
