classdef (Abstract) IntegAdamsBashforthMoultonAb < IntegMultiStepAb
    % An abstract class with common fuctionality for Adams - Bashforth -
    % Moulton family of predictor - corrector methods for numerical 
    % solving of differential equations. 
    %
    % It contains a common engine for all methods of this family.
    %
    % All classes based on Adams - Bashforth - Moulton methods should be 
    % derived from this one.
    
    properties (Access=protected)
        ab_coef;    % A vector of coefficients for the desired Adams - Bashforth predictor method
        am_coef;    % A vector of coefficients for the desired Adams - Moulton corrector method
    end % properties
    
    methods (Access=protected)
        function obj = IntegAdamsBashforthMoultonAb(model, input, t_start, t_stop, t_step, init_condition)
            % Constructor.
            %
            % Input:
    ...
    methods
        function output = run(self)
            ...
            % The common "engine" is used for all
            ...
            for t = upper_limit+self.t_step : self.t_step : self.t_stop-self.t_step,
                ...
                p = s + self.t_step * sd * self.ab_coef(1);
                for i = 2:N
                    p = p + self.t_step * self.ab_coef(i) * ...
                ...
                s = s + self.t_step * pd * self.am_coef(1);
                if ( Nam >=2 )
                    s = s + self.t_step * sd * self.am_coef(2);
                end %if
                for i = 3:Nam
                    s = s + self.t_step * self.am_coef(i) * ...
                        H(:, ((i-3)*STATE_COLS+1) : ((i-2)*STATE_COLS) );
                end %for
